...
# In Lab 5, anti_join() can be leveraged in validating our supect by: 

interview |> 
  anti_join(suspect,
          by = join_by(person_id == id))

# Since anti_join removes observations that are in both data set, our row number (4991) indicates that nothing has been removed and therefore no evidence of an interview with the suspect. 
...
I've created professional looking, reproducible analyses using RStudio projects, Quarto documents, and the here package in the following assignments:

. Labs 1 - 5 and Challenges 1 - 3

. Specifically labs 2 and 5
...
ggplot(data = surveys, 
  ...
  annotate('text', y = 1:14, x = 250, label = c('Neotoma',
                                                'Chaetodipus',
                                                'Peromyscus',
                                                'Perognathus',
                                                'Reithrodontomys',
...
Throughout the course, I successfully revise my thinking by reducing redundancies to ensure an efficient programming style. Through submitting every revision and analyzing the feedback by Dr. Theobold, I was able to embark on my critical thinking and research to determine different functions that can be implemented to tackle an objective from a variety of angles. In the code chunk below, I had originally developed two pipe operators - one to retrieve the maximum average student course failures and one for the minimum average student course failures in lab 3 question 11. However, I reduced 5 lines of code by applying the range() function that simultaneously calculates the max and min of numeric vectors to store both the minimum and maximum proportions of average student failures in the stu_fail object.
...
Within Lab 7 Peer Review, I provided the following feedback that I believe to be substantive and informative, both for me and my peer:

###### ***Tidy Code***

###### ***Amazing job applying ggplot within a pipe operator! The \|\> and “+” operator is applied perfectly at the end of each pipe and ggplot function which is awesome. Commas are applied appropriately followed by new lines to maintain a tidy code. Great use of non-traditional default colors, it definitely adds character. I might recommend widening the width of your visualization plot in question 2 so viewers don’t have to tilt their head.***

###### ***Efficient Code***

###### ***I don’t seem to see any redundancy in functions (no repeats) which is awesome! I also don’t seem to find creating multiple variables when not necessary which is great! Within number 4, I might recommend using the stopifnot() function so you can eliminate 4 lines for an efficient code chunk. Overall, awesome job!***

As a collaborator on weekly practice activities, I believe I have gained the confidence to share my perspective without the scare of being perceived as "uneducated" on the subject matter. Additionally, I further learned to trust my peers' problem-solving style that may differ from my own which equipped me with unparalleled knowledge and perspective of pursuing solutions in variety of ways.
